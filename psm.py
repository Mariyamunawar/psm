...
#page styling 
st.set_page_config(page_title="Password Strength Checker By Mariya Munawar",page_icon="🌘", layout="centered") 
#custom css 
st.markdown(""" 
<style> 
    .main {text-align: center;} 
    .stTextInput {width: 60% !important; margin: auto; } 
    stButton button {width: 50%; background-color #ACAF50; color: white; front-size: 18px; } 
    .stButton button:hover {background-color: #5a049;}
<style>
...
def check_password_strength(password): 
    ...
    if len(password) >= 8: 
        ...
        feedback.append("❌ password should be **atleast 8 charactar long**.") 

    if re.search(r"[A-Z]", password) and re.search(r"[a-z]", password): 
        score += 1 
    else: 
        feedback.append("❌ password should include **both upper case (A_Z) and lower case (a-z)letters**.") 
    ...
    if feedback: 
        with st.expander("🔍**Improve your passwor** "): 
            for item in feedback: 
                st.write(item) 
password = st.text_input("Enter your password:", type="password", help="Ensure your password is storng 🔐") 
